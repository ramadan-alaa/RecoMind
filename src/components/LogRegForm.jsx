...
const LogRegForm = ({ title }) => {
  ...
  const { isloading: signupLoading, success: signupSuccess } = useSelector(
    (state) => state.signup
  );
  const { isloading: signinLoading, isAuthenticated } = useSelector(
    (state) => state.signin
  );
  ...
  }, [signupSuccess, isSignUp, navigate]);
      ...
      <div className="container flex flex-col md:flex-row md:justify-center gap-12 mt-12 md:mt-12 mx-auto h-[90vh] items-center">
        ...
        <div className="flex-1 flex flex-col gap-4 w-full max-w-[550px] bg-[var(--Primary)] p-6 sm:p-10 md:px-[74px] md:py-[34px] rounded-3xl shadow-2xl">
          ...
          >
            ...
            >
              {isloading ? (
                <span className="animate-spin h-6 w-6 border-2 border-[var(--Primary)] border-t-transparent rounded-full"></span>
              ) : (
                title
              )}
            </button>
