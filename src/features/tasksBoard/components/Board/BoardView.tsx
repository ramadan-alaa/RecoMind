...
const PERSONAL_COLUMNS: { status: TaskStatus; label: string; dotColor: string }[] = [
  { status: "overdue", label: "Overdue", dotColor: "#df5d5d" },
  { status: "todo",    label: "To-Do",   dotColor: "#7ee3ff" },
...
const BoardView: React.FC = () => {
  const { tasks, activeBoard } = useSelector((s: RootState) => s.tasks);

  const columns = activeBoard === "plans" ? PLANS_COLUMNS : PERSONAL_COLUMNS;
  const boardTasks = tasks.filter((t) => t.boardType === activeBoard);

  return (
    <div className="flex gap-4 flex-1 overflow-x-auto pb-4" style={{ minHeight: 0 }}>
      {columns.map((col) => (
        <KanbanColumn
          key={col.status}
          status={col.status}
